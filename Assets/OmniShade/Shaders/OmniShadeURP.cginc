	...
	half3 col_diffuse = 0;
		...
		LIGHT_LOOP_BEGIN(lightsCount)
			...
			#endif
			{
				...
				col_diffuse += light.color * light.distanceAttenuation * col_shadow;
			}
		LIGHT_LOOP_END
	#endif
	return col_diffuse;
...
half3 AdditionalLightsVert(half3 pos_world, half3 nor_world, half4 pos_clip, half _DiffuseWrap, half _DiffuseBrightness, half _DiffuseContrast, uint meshRenderingLayers, half3 shadowColor) {
	half3 col_diffuse = 0;
	#if _ADDITIONAL_LIGHTS
		uint lightsCount = GetAdditionalLightsCount();
		#if USE_FORWARD_PLUS
...
half3 AdditionalLightsVert(half3 pos_world, half3 nor_world, half4 pos_clip, half _DiffuseWrap, half _DiffuseBrightness, half _DiffuseContrast, uint meshRenderingLayers, half3 shadowColor) {
	return AdditionalLightsFrag(pos_world, nor_world, pos_clip, _DiffuseWrap, _DiffuseBrightness, _DiffuseContrast, meshRenderingLayers, shadowColor);
}
