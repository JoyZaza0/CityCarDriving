...
//////////////////////////////////////////////////////////////////////////////////////////
	...
	struct v2f {
		...
		float3 pos_world : TEXCOORD1;
		float3 nor_world : TEXCOORD2;
		#if SPECULAR && !(NORMAL_MAP || NORMAL_MAP2 || NORMAL_MAP_TOP) && !(DIRLIGHTMAP_COMBINED && LIGHTMAP_ON) && !FLAT
			float3 specReflectDir : TEXCOORD3;
		#endif
		#if RIM || SPECULAR || REFLECTION
			float3 viewDir_world : TEXCOORD4;
...
v2f vert (appdata_full v) {
	...
	o.nor_world = UnityObjectToWorldNormal(v.normal);
		...
		float4 pos_clip = 0;
			...
			pos_clip = o.pos_clip;
		#endif
		o.col_diffuse_add = AdditionalLightsVert(o.pos_world, o.nor_world, pos_clip, _DiffuseWrap, _DiffuseBrightness, _DiffuseContrast, meshRenderingLayers, _ShadowColor.rgb);
	#endif
...
half4 frag (v2f i) : COLOR {
	...
	i.nor_world = normalize(i.nor_world);
		...
		#if TRIPLANAR_LOCAL
			float3 triPos = i.pos_local;
		#else
			float3 triPos = i.pos_world.xyz;
		#endif
		#if TRIPLANAR_BASE_UV
			float2 uvX = uv;
			float2 uvZ = uv;
		#else
			float2 uvX = triPos.zy;
			float2 uvZ = triPos.xy;
		#endif
		float2 uvY = triPos.xz;
		float4 blend;
	...
	uint meshRenderingLayers = GetMeshRenderingLightLayerCustom();
			...
			#endif
			#if NORMAL_MAP2
				norX += UnpackNormal(tex2D(_NormalTex2, TRANSFORM_TEX(uvX, _NormalTex2))) * _NormalStrength;
				norYbottom += UnpackNormal(tex2D(_NormalTex2, TRANSFORM_TEX(uvY, _NormalTex2))) * _NormalStrength;
				norZ += UnpackNormal(tex2D(_NormalTex2, TRANSFORM_TEX(uvZ, _NormalTex2))) * _NormalStrength;
			#endif
		...
		half3 col_diffuse = 0;
			...
			if (IsLitMainLight(unity_LightData, meshRenderingLayers, false)) {
				...
				float4 pos_clip = 0;
					...
					pos_clip = i.pos_clip;
				#endif
				col_diffuse += AdditionalLightsFrag(i.pos_world, adjNor_world, pos_clip, _DiffuseWrap, _DiffuseBrightness, _DiffuseContrast, meshRenderingLayers, _ShadowColor.rgb);
			#endif
		#endif
