...
public class CCDS_SoundtrackManager : ACCDS_Manager {
    ...
    private void Awake() {
        ...
        SetMusicVolume(PlayerPrefs.GetFloat(CCDS_Settings.Instance.playerPrefsMusicVolume, CCDS_Settings.Instance.defaultMusicVolume));

        //  Setting the general volume level with latest saved value.
        SetGeneralVolume(PlayerPrefs.GetFloat(CCDS_Settings.Instance.playerPrefsAudioVolume, CCDS_Settings.Instance.defaultAudioVolume));
    ...
    private void CCDS_Events_OnAudioChanged() {
        ...
        //  Getting the music volume level with latest saved value.
        SetMusicVolume(PlayerPrefs.GetFloat(CCDS_Settings.Instance.playerPrefsMusicVolume, CCDS_Settings.Instance.defaultMusicVolume));

        //  Getting the general volume level with latest saved value.
        SetGeneralVolume(PlayerPrefs.GetFloat(CCDS_Settings.Instance.playerPrefsAudioVolume, CCDS_Settings.Instance.defaultAudioVolume));
