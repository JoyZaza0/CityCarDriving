...
public class RCCP_Recorder : RCCP_Component {
    ...
    public class VehicleInput {
        ...
        public VehicleInput(float _gasInput, float _brakeInput, float _steerInput, float _handbrakeInput, float _clutchInput, float _boostInput, int _direction, int _currentGear, bool _NGear, bool _lowBeamHeadLightsOn, bool _highBeamHeadLightsOn, bool _indicatorsLeft, bool _indicatorsRight, bool _indicatorsAll) {
    ...
    private IEnumerator Replay() {
        ...
        for (int i = 0; i < recorded.inputs.Length && mode == RecorderMode.Play; i++) {
            ...
            if (CarController.Gearbox) {
                ...
                CarController.Gearbox.OverrideGear(recorded.inputs[i].currentGear, recorded.inputs[i].direction == 1 ? false : true, recorded.inputs[i].NGear);
